...
- **State persistence.** Lead writes `.maw-lead-state.json` at phase transitions for compaction survival.
...
| `maw-implementer` | Autonomous coder | Sonnet | `Task(subagent_type: "maw-native:maw-implementer")` |
| `maw-tester` | Test writer | Sonnet | `Task(subagent_type: "maw-native:maw-tester")` |
| `maw-reviewer` | Quality gate (read-only) | Sonnet | `Task(subagent_type: "maw-native:maw-reviewer")` |
| `maw-fixer` | Targeted fixes | Sonnet | `Task(subagent_type: "maw-native:maw-fixer")` |
